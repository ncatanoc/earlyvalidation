...
public class DELETE_OBSTACLE extends Thread{
	...
	public DELETE_OBSTACLE(ref1_kinectic m) {
		this.machine = m;
	}
	...
	/*@ public normal_behavior
		requires true;
 		assignable \nothing;
		ensures \result <==> machine.get_obstacles().has(Obs) && !machine.get_vehicles().has(Obs); */
	public /*@ pure */ boolean guard_DELETE_OBSTACLE( Integer Obs) {
		return machine.get_obstacles().has(Obs) && !machine.get_vehicles().has(Obs);
	}
